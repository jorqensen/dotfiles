...
if ! command -v fish &> /dev/null; then
    . /etc/os-release
    echo "deb http://download.opensuse.org/repositories/shells:/fish:/release:/4/${ID^}_${VERSION_ID}/ /" | sudo tee /etc/apt/sources.list.d/shells:fish:release:4.list
    curl -fsSL "https://download.opensuse.org/repositories/shells:fish:release:4/${ID^}_${VERSION_ID}/Release.key" | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/shells_fish_release_4.gpg > /dev/null
    sudo apt update
